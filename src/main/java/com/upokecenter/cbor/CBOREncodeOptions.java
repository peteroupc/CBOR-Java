...
@Deprecated
     ...
     * {@code EncodeToBytes()}, {@code toString()}, {@code ToJSONString()}, or
     * {@code WriteTo}), because object references are expanded in the
     ...
     * 28([29(5), 29(5)])]}, expands to a CBOR object with a serialized size of
     * about 1831 bytes when this property is enabled, as opposed to about 69 bytes
     * when this property is disabled.</p> <p>One way to mitigate security issues
     ...
     * so-called "limited memory stream" to the {@code WriteTo} or {@code
     * WriteJSONTo} methods when serializing the object to JSON or CBOR. A "limited
     * memory stream" is a {@code InputStream} (or {@code OutputStream} in Java) that
     ...
     * CBORObject.WriteTo} or {@code CBORObject.WriteJSONTo} for example code.)
