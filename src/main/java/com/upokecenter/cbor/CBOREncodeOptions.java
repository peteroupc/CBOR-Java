package com.upokecenter.cbor;
  ...
  public final class CBOREncodeOptions {
     ...
     * com.upokecenter.cbor.CBOREncodeOptions} class with all the default options.
     */
...
@Deprecated
     ...
     * then re-encoded.</p><p><b>About sharable objects and references</b></p>
     ...
     * the string would save space.</p> <p>Note that unlike most other tags, these
     ...
     * those keys or values.</p> <p><b>Security Note</b></p> <p>When this property
     ...
     * service when the decoded CBOR object is then serialized (e.g., with {@code
     * EncodeToBytes()}, {@code toString()}, {@code ToJSONString()}, or {@code
     * WriteTo}), because object references are expanded in the process.</p>
     * <p>For example, the following object in CBOR diagnostic notation, {@code
     * [28(["xxx", "yyy"]), 28([29(0), 29(0), 29(0)]), 28([29(1), 29(1)]),
     * 28([29(2), 29(2)]), 28([29(3), 29(3)]), 28([29(4), 29(4)]), 28([29(5),
     * 29(5)])]}, expands to a CBOR object with a serialized size of about 1831
     * bytes when this property is enabled, as opposed to about 69 bytes when this
     * property is disabled.</p> <p>One way to mitigate security issues with this
     * property is to limit the maximum supported size a CBORObject can have once
     * serialized to CBOR or JSON. This can be done by passing a so-called "limited
     * memory stream" to the {@code WriteTo} or {@code WriteJSONTo} methods when
     * serializing the object to JSON or CBOR. A "limited memory stream" is a
     * {@code InputStream} (or {@code OutputStream} in Java) that throws an exception if
     * it would write more bytes than a given maximum size or would seek past that
     * size. (See the documentation for {@code CBORObject.WriteTo} or {@code
     * CBORObject.WriteJSONTo} for example code.) Another mitigation is to check
     * the CBOR object's type before serializing it, since only arrays and maps can
     * have the security problem described here, or to check the maximum nesting
     * depth of a CBOR array or map before serializing it.</p>
...
private final boolean propVarallowempty;
     ...
     * property is {@code true} and a CBOR map has two or more values with the same
     * key, the last value of that key set forth in the CBOR map is taken.
...
private final boolean propVarfloat64;
     ...
     * form.</li></ul> In this form, CBOR tags are not used, map keys are written
     ...
     * to the FIDO Client-to-Authenticator Protocol 2 specification.
     * @return {@code true} if CBOR objects are written out using the CTAP2
     * canonical CBOR encoding form; otherwise, {@code false}. The default is
     * {@code false}.
     */
