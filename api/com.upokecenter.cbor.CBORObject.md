...
<p>Generates a CBORObject from an arbitrary object, using the given options
 ...
 key's name is adjusted according to the rules described in the <code>PODOptions</code> documentation.</li></ul>
 ...
 supplied data unless the application ensures that the possible set of
 <paramref name='obj'/> inputs to this method is limited to types
 specially handled by this method (such as <code>int</code> or
 <code>String</code>) and/or to plain-old-data types (POCO or POJO types)
 ...
 criterion above.</p> <p>REMARK: The behavior of this method is likely
 to change in the next major version (4.0). There are certain
